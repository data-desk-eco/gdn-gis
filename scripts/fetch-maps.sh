...
# 4. scrape the per-edition zip password off the download page, into a .zip-password
# file beside the zip (where gdn-gis reads it), before pulling the ~3gb bundle.
...
[ -n "$pw" ] || { printf %s "$dl" > download-page.html
  echo "fetch-maps: no password found on download page (dumped to download-page.html)" >&2; exit 1; }
printf %s "$pw" > "$(dirname "$out")/.zip-password"
echo "fetch-maps: zip password $pw -> $(dirname "$out")/.zip-password"
