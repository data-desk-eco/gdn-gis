...
// rows south→north.
//
//   terr0.bin  resident coarse relief: 1801×1801 posts at 200 m over the whole
//              grid, from os terrain 50 (data/terr50.zip, fetch-terrain.sh).
//   terr1.bin  paged detail: 251×251 posts at 8 m per populated cell, from the
//              ea 1 m composite dtm crawl (data/terr1/<cellid>, fetch-lidar.py).
//              fixed-size records in ascending cell order; nodata holes filled
//              from the os 50 m grid so the client never sees a sentinel.
...
pub(crate) const N0: usize = 1801; // coarse posts per grid edge
...
impl Master {
    fn load(m: &MapCfg) -> Master {
        ...
        let n = (m.ncols.max(m.nrows) as f64 * m.cell_km * 1000.0 / 50.0) as usize + 1;
        let mut g = vec![1000u16; n * n]; // default sea level
        ...
        for i in 0..arch.len() {
            ...
            while let Some(k) = it.next() {
                if k.parse::<f64>().is_ok() {
                    ...
                    if xll + nc as f64 * cs <= minx || xll >= minx + n as f64 * 50.0
                        || yll + nr as f64 * cs <= miny || yll >= miny + n as f64 * 50.0 {
                        break;
                    }
                    ...
                    for r in 0..nr {
                        ...
                        if gy < 0.0 || gy >= n as f64 {
                            continue;
                        }
                        for c in 0..nc {
                            let x = xll + c as f64 * cs;
                            let gx: f64 = ((x - minx) / 50.0).round();
                            if gx < 0.0 || gx >= n as f64 {
                                continue;
                            }
                            let v = vals[r * nc + c];
                            if v > -100.0 && v < 3000.0 {
                                g[gy as usize * n + gx as usize] = (v * 10.0 + 1000.0).round() as u16;
                            }
                        }
                    }
                    break;
        ...
        Master { g, n, minx, miny }
    }
    /// bilinear elevation (u16 dm+1000) at bng metres.
    fn at(&self, x: f64, y: f64) -> u16 {
        let (gx, gy) = (((x - self.minx) / 50.0).clamp(0.0, (self.n - 1) as f64 - 1e-9),
                        ((y - self.miny) / 50.0).clamp(0.0, (self.n - 1) as f64 - 1e-9));
        let (x0, y0) = (gx as usize, gy as usize);
        let (dx, dy) = (gx - x0 as f64, gy - y0 as f64);
        let v = |i: usize, j: usize| self.g[(y0 + j).min(self.n - 1) * self.n + (x0 + i).min(self.n - 1)] as f64;
...
pub fn write(m: &MapCfg) {
    ...
    for j in 0..N0 {
        for i in 0..N0 {
            w.write_all(&master.g[(j * 4).min(master.n - 1) * master.n + (i * 4).min(master.n - 1)].to_le_bytes()).unwrap();
        }
    }
    w.flush().unwrap();
    ...
    for cid in 0..ncell {
        let Ok(raw) = std::fs::read(format!("data/terr1/{cid}")) else { continue };
        if raw.len() != P * P * 2 {
    ...
    eprintln!("  terrain: terr0 {N0}×{N0} posts, terr1 {ncells} cells ({nfill} posts backfilled)  ->  {}/terr*", m.dir);
}
